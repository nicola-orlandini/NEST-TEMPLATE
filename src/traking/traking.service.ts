...
export class TrakingService {
    ...
    async getBarcode(barcode: string) {
        const pack = await this.fermoPointPacchiRepository.findOne({ where: { barcode_pacco: barcode } })
        if (!pack) {
            throw new HttpException(`barcode ${barcode} non trovato`, HttpStatus.NOT_FOUND)
        }
        ...
        if (Object.keys(trakings).length <= 1) {
            throw new HttpException(`traking ${barcode} incompleto`, HttpStatus.CONFLICT)
        }
        const alfredStatusExclude = ['P2P02', 'P2POK', 'P2POKR', 'P2POK2', 'P2PNOGEST', 'P2PNDR']
        ...
        for (const index in trakings) {
            ...
            const fromStatus = status.find((item) => { return item.code == trakings[parseInt(index)].alfred_status }).it
            const toStatus = status.find((item) => { return item.code == trakings[parseInt(index) + 1].alfred_status }).it
            ...
            timingLines.push({
                ...
                from: { ...trakings[parseInt(index)], alfred_status_string: fromStatus },
                to: { ...trakings[parseInt(index) + 1], alfred_status_string: toStatus }
            })
        }
